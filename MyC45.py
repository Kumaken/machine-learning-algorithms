...
class Node:
    def __init__(self,
                _parent = None, # A Node
                _children = None, # list of NODES -> MUTABLE DEFAULT VALUES ARE ROOT OF ALL EVIL
                _infoGain = None, # double?
                _attribute = None, # string?
                _valuesTaken = None): # rules chosen (decision made in each nodes of the parent)
        self.parent = _parent
        self.children = _children or []
        self.infoGain = _infoGain
        self.attribute = _attribute
        self.valuesTaken = _valuesTaken or {}
...
class MyTree:
    ...
    def buildTree(  self,
        ...
        if self.entropyData(dataset) == 0.0 :
            ...
            curr_node.attribute = self.targetAttribute
            curr_node.valuesTaken[curr_node.attribute] = self.mostValue(dataset, curr_node.attribute)
            curr_node.children = []
            return
        ...
        vals_set = self.getValuesInAttribute(dataset, best_node[0])
        # print(best_node[0], vals_set)
    ...
    def predict(self, values): # (values = {} dict of facts provided)
        ...
        while(not curr_node.isLeaf()):
            ...
            for child in curr_node.children:
                ...
                # print(child.valuesTaken[curr_attr], threshold_dict[curr_node.attribute], target_val)
                # print(child.valuesTaken)
                if isinstance(target_val,float):
                    if float(target_val) >= float(threshold_dict[curr_node.attribute]):
                        # find child with class.value = 1.0

                        if str(child.valuesTaken[curr_attr]) == "1.0":
                            curr_node = child
                            hit = True
                            break

                    else: # <
                        # find child with class.value = 0.0
                        if str(child.valuesTaken[curr_attr]) == "0.0":
                            curr_node = child
                            hit = True
                            break
                else:
            ...
            if not hit:
                # at this point, the sequence is not recognized in training dataset:
                return "LOST CAUSE"
...
def resetThresholdDict():
    global threshold_dict
    threshold_dict = dict()

def printThresholdDict():
